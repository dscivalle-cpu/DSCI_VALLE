...
//
// Idealmente Kobo envia el form_id configurado en el XLSForm dentro de
// _xform_id_string. En la practica, algunas instancias de KoboToolbox
// envian ahi el UID interno del proyecto en su lugar (confirmado en
// pruebas reales). Por eso usamos el form_id como primer intento, y si
// no coincide con ninguno, identificamos el formulario por la presencia
// de un campo que solo existe en ese formulario especifico.
// ---------------------------------------------------------------------
function identificarFormulario(body: any): "ORGANIZACIONES" | "ASOCIADOS" | "CERTIFICACION" | null {
  const xformId = body["_xform_id_string"];
  if (xformId === FORM_IDS.ORGANIZACIONES) return "ORGANIZACIONES";
  if (xformId === FORM_IDS.ASOCIADOS) return "ASOCIADOS";
  if (xformId === FORM_IDS.CERTIFICACION) return "CERTIFICACION";

  // Fallback por campos unicos (orden importa: el mas especifico primero)
  if (body["numero_documento_productor"] !== undefined) return "CERTIFICACION";
  if (body["representante_legal"] !== undefined) return "ORGANIZACIONES";
  if (body["numero_documento"] !== undefined) return "ASOCIADOS";

  console.error("No se pudo identificar el formulario. _xform_id_string recibido:", xformId);
  return null;
}
...
Deno.serve(async (req: Request) => {
  ...
  const tipoFormulario = identificarFormulario(body);
  ...
  try {
    ...
    switch (tipoFormulario) {
      ...
      case "CERTIFICACION":
        ...
        break;
      default:
        console.error("Formulario no reconocido. _xform_id_string:", body["_xform_id_string"]);
        return new Response(
          `Formulario no reconocido (_xform_id_string: ${body["_xform_id_string"]})`,
          { status: 400 },
        );
    }
